"""
LAA Uplift Narrative Generator - Version 0.1
...
class NarrativeGenerator:
    ...
    def generate_narrative(self, data):
        ...
        # Header
        narrative.append(("Claim For Additional General Enhancement", "title"))
        ...
        # First paragraph - the claim
        intro_first = f"An enhancement of {uplift}% is claimed on applicable items within the {matter_type} work."
        narrative.append((intro_first, "normal"))
        narrative.append(("", "normal"))
        
        # Second paragraph - justification
        intro_second = (f"This relates to the matter of {case_name} and in our opinion, this uplift is "
                       f"considered and justifiable: Exceptional factors detailed below meet the requirements "
                       f"in CPR 44.4(3). These factors also satisfy the Legal Aid Agency's Costs Assessment "
                       f"Guidance (Version 1a, 23 September 2024). The enhancement applies only to eligible "
                       f"work items and excludes standard administrative tasks and routine communications as "
                       f"per LAA guidelines.")
        narrative.append((intro_second, "normal"))
        narrative.append(("", "normal"))
        ...
        narrative.append(("Summary of Justification", "heading"))
    ...
    def generate_word_document(self, data):
        ...
        font_name = "Arial Nova Cond Light"
        # If Arial Nova Cond Light is not available, fall back to Arial
        fallback_font = "Arial"
        
        for text, style in narrative:
            if style == "title":
                p = doc.add_paragraph()
                p.alignment = WD_ALIGN_PARAGRAPH.LEFT  # Left align
                p.space_before = Pt(0)
                p.space_after = Pt(12)  # Add space after title
                run = p.add_run(text)
                ...
                p.space_before = Pt(12)  # Add space before heading
                p.space_after = Pt(6)
                run = p.add_run(text)
                run.font.name = font_name
                run.font.size = Pt(12)
                ...
                p.alignment = WD_ALIGN_PARAGRAPH.LEFT
                run = p.add_run(text[2:])  # Remove bullet point as Word adds its own
                run.font.name = font_name
                run.font.size = Pt(12)
                ...
                if text:  # Only add paragraph if there's text
                    p = doc.add_paragraph()
                    p.alignment = WD_ALIGN_PARAGRAPH.LEFT
                    p.space_after = Pt(6)  # Add space between paragraphs
                    run = p.add_run(text)
                    run.font.name = font_name
                    run.font.size = Pt(12)
        
        # Try to set document-wide font
        try:
            doc.styles['Normal'].font.name = font_name
            doc.styles['Normal'].font.size = Pt(12)
        except:
            # If Arial Nova Cond Light isn't available, use Arial
            try:
                doc.styles['Normal'].font.name = fallback_font
                doc.styles['Normal'].font.size = Pt(12)
            except:
                pass
...
class UpliftNarrativeApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(f"LAA Uplift Narrative Generator v{VERSION} - Woodruff Billing Ltd")
        ...
        # Version label
        version_label = tk.Label(main_frame, text=f"Version {VERSION} - {RELEASE_DATE}", 
        ...
        # Create buttons with simpler styling for better Windows compatibility
        button_style = {
            'font': ('Arial', 10),
            'padx': 20,
            'pady': 8,
        ...
        self.copy_btn = tk.Button(button_frame, 
                                  text="Copy to Clipboard (for Word)", 
        ...
        self.save_word_btn = tk.Button(button_frame, 
