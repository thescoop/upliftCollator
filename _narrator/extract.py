"""Extract solicitor answers from a summary generated by the Uplift Collator.

Two formats. Since v1.13 the Collator downloads a **.docx**, read by
``extract_docx.py`` against a paragraph-equality contract; every matter begun
earlier holds a **PDF**, which this module reads and must keep reading
forever. ``extract_formdata``, ``diagnose`` and ``explain_empty_extraction``
accept either format and dispatch on content (see ``detect_format``).

Everything below this docstring is the PDF path. Its structure was produced
by the retired ``generatePdfSummary`` in ``script.js`` via
...
def explain_empty_extraction(path: str | Path) -> str:
    """Plain-English reason an extraction came back empty — either format.

    Built from the matching :func:`diagnose`, so it reports structure only —
    never a field value, an explanation, or any other client text. Safe to
    print to a terminal, paste into a message, or write into a log.
    """
    if detect_format(path) == "docx":
        from extract_docx import explain_empty_extraction_docx

        return explain_empty_extraction_docx(path)
    return explain_empty_extraction_pdf(path)


def explain_empty_extraction_pdf(pdf_path: str | Path) -> str:
    """Plain-English reason a PDF extraction came back empty."""
...
# ── Format dispatch ────────────────────────────────────────────────────────
#
# Since v1.13 the Collator downloads a .docx; every earlier matter holds a
# PDF, and those must keep reading correctly forever. The public entry points
# below (extract_formdata, diagnose, explain_empty_extraction) accept either
# and dispatch; the docx logic lives in extract_docx.py against its own,
# stricter contract, and the PDF path here is frozen as the legacy path.
#
# Content decides before the extension does: a PDF begins "%PDF", a .docx is
# a zip beginning "PK". A renamed file therefore still reads as what it *is*,
# and only a file that is neither falls back to its extension so the right
# format's diagnostics can say what is wrong with it.
...
def detect_format(path: str | Path) -> str:
    """Return "docx" or "pdf" for *path*, by content first, extension second."""
    path = Path(path)
    try:
    ...
    if head.startswith(_PDF_MAGIC):
        return "pdf"
    return "docx" if path.suffix.lower() == ".docx" else "pdf"


def extract_formdata(path: str | Path) -> dict:
    """Read a Collator-generated summary (.docx or PDF) → formData equivalent."""
    if detect_format(path) == "docx":
        from extract_docx import extract_formdata_docx

        return extract_formdata_docx(path)
    return extract_formdata_pdf(path)


def extract_formdata_pdf(pdf_path: str | Path) -> dict:
...
def diagnose(path: str | Path) -> dict:
    """Structural diagnostics for an extraction failure — either format."""
    if detect_format(path) == "docx":
        from extract_docx import diagnose_docx

        return diagnose_docx(path)
    return diagnose_pdf(path)


def diagnose_pdf(pdf_path: str | Path) -> dict:
    """Structural diagnostics for a PDF extraction failure.
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 3 or (
        ...
        print(f"Usage: {sys.argv[0]} [--debug] <docx-or-pdf-path>", file=sys.stderr)
        sys.exit(1)
    if len(sys.argv) == 3:
